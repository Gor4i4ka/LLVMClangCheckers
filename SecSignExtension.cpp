//== SecSignExtension.cpp - Second sign extension---*- C++ -*--==//
//===----------------------------------------------------------------------===//
///
/// \file
/// This defines SecSignExtension, a check that warns about
/// a subvariety of possible overflow errors in pesent in system of 32-bit and
/// absent in system of 64-bit and vice-versa
///
...
struct TypeInfo {
	...
	TypeInfo(std::string str, int size32, int size64) {
		typeName = str;
		typeSize32 = size32;
		typeSize64 = size64;
	}
...
bool FindList(QualType qtype, int *bit32, int *bit64) {
	...
	if (castType && exprType) {
		if (foundCast && foundExpr) {
			if (castType->isSignedIntegerType()
					&& exprType->isUnsignedIntegerType()) {
				if ((castSize32 / exprSize32 < 2
						&& !(castSize64 / exprSize64 < 2))
						|| (castSize64 / exprSize64 < 2
								&& !(castSize32 / exprSize32 < 2))) {
					return true;
				}
			}
			if ((castType->isSignedIntegerType()
...
class CastWalk: public StmtVisitor<CastWalk> {
	BugReporter &BR;
...
class FuncWalk: public RecursiveASTVisitor<FuncWalk> {
	BugReporter &BR;
	const CheckerBase *Checker;
	AnalysisDeclContext *AC;
...
public:
	...
	explicit FuncWalk(BugReporter &B, const CheckerBase *Checker,
			AnalysisDeclContext *A) :
			BR(B), Checker(Checker), AC(A) {
	}
	// The methods to check the declared type and return type of a function to follow the abovementioned rules
...
void FuncWalk::ThrowError(Stmt *S) {
	...
	BR.EmitBasicReport(AC->getDecl(), Checker, "Overflow 32-bit/64-bit",
			"SecSignExtension", "Overflow 32-bit/64-bit", ELoc, R);
}

// Storing function's return type for future children (ReturnStmt).
bool FuncWalk::VisitFunctionDecl(FunctionDecl *FD) {
	funcqt = FD->getDeclaredReturnType();
	return true;
}

// Getting the actual return value type and comparing it to the declared one.
bool FuncWalk::VisitReturnStmt(ReturnStmt *RS) {
...
void CastWalk::VisitCallExpr(CallExpr *CE) {
...
void CastWalk::ThrowError(Stmt *S) {
	//A general method to "throw" a warning
	SourceRange R = S->getSourceRange();
	PathDiagnosticLocation ELoc = PathDiagnosticLocation::createBegin(S,
			BR.getSourceManager(), AC);
...
void CastWalk::VisitChildren(Stmt *S) {
	for (Stmt *Child : S->children())
		if (Child)
			Visit(Child);
}

void CastWalk::VisitStmt(Stmt *S) {
	VisitChildren(S);
}

// Here only assignment operations are taken into consideration,
// as the most obvious mistakes easy to fix.
// Bitwise operations are left as prone to being probably false or safe cases.
// Call expressions are also managed separately.
void CastWalk::VisitBinaryOperator(BinaryOperator *BN) {
...
void SecSignExtension::checkASTCodeBody(const Decl *D, AnalysisManager &AM,
		BugReporter &BR) const {
	ListsInit();
...
// Registration
void ento::registerSecSignExtension(CheckerManager &Mgr) {
	Mgr.registerChecker<SecSignExtension>();
}

bool ento::shouldRegisterSecSignExtension(const LangOptions &LO) {
	return true;
}
