...
public:
	explicit WalkAST(BugReporter &B, const CheckerBase *Checker,
...
void WalkAST::ThrowError(Stmt *S) {
	...
	BR.EmitBasicReport(AC->getDecl(), Checker,
			"Usage of an implicit function in address calculations",
			"SEC_FUNC_WITHOUT_PROTOTYPE",
			"Usage of an implicit function in address calculations", ELoc, R);
}

void WalkAST::VisitBinaryOperator(BinaryOperator *bn) {
...
void SecFuncWithoutPrototype::checkASTCodeBody(const Decl *D,
...
// Registration
void ento::registerSecFuncWithoutPrototype(CheckerManager &Mgr) {
	Mgr.registerChecker<SecFuncWithoutPrototype>();
}

bool ento::shouldRegisterSecFuncWithoutPrototype(const LangOptions &LO) {
	return true;
}
