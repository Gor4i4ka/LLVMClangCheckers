...
bool WalkAST::checkType(QualType qt) {
	...
	std::regex regt(
			"(.*)8(.*)|(.*)16(.*)|(.*)32(.*)|(.*)64(.*)|(.*)128(.*)|(.*)size_t(.*)|(.*)ptr(.*)|(.*)long long(.*)");
...
bool WalkAST::VisitCXXMethodDecl(CXXMethodDecl *S) {
	...
	if (fd) {
		...
		// Checking whether its the dangerous pattern and throwing an error if it is.
		if (!str1.compare(str2)) {
			...
			if (!checkType(qt)) {
				...
				BR.EmitBasicReport(AC->getDecl(), Checker,
						"index operator overload bit error possible",
...
void SquareBracketsChecker::checkASTCodeBody(const Decl *D, AnalysisManager &AM,
		BugReporter &BR) const {
	WalkAST Walker(BR, this, AM.getAnalysisDeclContext(D));
	Walker.TraverseDecl(const_cast<Decl*>(D));
}

void ento::registerSquareBracketsChecker(CheckerManager &Mgr) {
	Mgr.registerChecker<SquareBracketsChecker>();
}

bool ento::shouldRegisterSquareBracketsChecker(const LangOptions &LO) {
	return true;
}
