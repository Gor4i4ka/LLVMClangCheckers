...
/// usage of any constant in the specified black list in multiple patterns.
///
//===−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−===//
// Implementation
...
namespace {
	...
	void 
	ThrowError (Stmt *S , Decl *D, AnalysisDeclContext *AC, BugReporter &BR, const CheckerBase *Checker) {
		if (!S && !D)
			return;
		...
		if (S) {
			...
			BR.EmitBasicReport(AC−>getDecl(), Checker, "Usage of a system−dependent type together with a 32−bit constant", 
					"MagicConstant", "Usage of a system−dependent type together with a 32−bit constant ", ELoc, R);
		}
		if (D) {
			SourceRange R = D−>getSourceRange();
			PathDiagnosticLocation ELoc = PathDiagnosticLocation::createBegin (D, BR.getSourceManager(), AC);
			BR.EmitBasicReport(AC−>getDecl(), Checker, "Usage of a system−dependent type together with a 32−bit constant", 
					"MagicConstant", "Usage of a system−dependent type together with a 32−bit constant ", ELoc, R);
		}
	}
	...
	CheckType (QualType qt, bool *FL) {
		...
		if (!str.empty()) {
			...
			*FL = true;
			...
			if (*FL)
				if (TD)
					CheckType(TD−>getDecl()−>getCanonicalDecl()−>getUnderlyingType(), FL);
		}
	}
	...
	void 
	SearchDanger (Stmt *S , Decl *D, bool *FL) {
		QualType qt;
		...
		// The code checks a contstant’s presence in the specified black list,
		// subsequently comparing it with every its member.
		{
			if (IntegerLiteral *IL = dyn_cast<IntegerLiteral>(S)) {
				...
				char *valp;
				char *lsvalp;
				for (auto iter = constsList.begin(); iter != constsList.end(); iter = std:: next(iter, 1)) {
					if (*FL) 
						break;
...
// A general method to check an expression f o r dangerous constant
// with an in−depth search .
// BitwiseOps and CallExprs are neglected .

	void 
	CheckChildren (Expr *E) {
...
//===−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−===//
	class ExprWalk: public StmtVisitor<ExprWalk> {
		...
		explicit ExprWalk( BugReporter &B, const CheckerBase *Checker, AnalysisDeclContext *A): BR(B), Checker(Checker), AC(A) {}
		// The Necessary methods to recursively walk through AST
		void VisitChildren ( Stmt *S);
		void VisitStmt ( Stmt *S);
		// The method f o r checking potentionally relevant Expressing statements
...
// variable and fielddeclarations
//===−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−−===//
	class VarWalk: public RecursiveASTVisitor<VarWalk> {
		...
		public :
		explicit VarWalk(BugReporter &B, const CheckerBase *Checker, AnalysisDeclContext *A): BR(B), Checker(Checker), AC(A) {}
...
bool 
VarWalk:: VisitVarDecl (VarDecl *VD) {
...
void 
ExprWalk::VisitDeclStmt(DeclStmt *DS) {}

void 
ExprWalk::VisitExpr(Expr *EX) {
	FlagsFlush();
...
void 
ExprWalk::VisitChildren(Stmt *S) {
	// the required VisitChildren method necessary to use V i s i t
...
void 
MagicConstant::checkASTCodeBody(const Decl *D, AnalysisManager &AM, BugReporter &BR) const {
	ListsInit();
...
// Registration
void 
ento::registerMagicConstant(CheckerManager &Mgr) {
Mgr.registerChecker<MagicConstant>();
}

bool 
ento::shouldRegisterMagicConstant(const LangOptions &LO) {
return true;
}
