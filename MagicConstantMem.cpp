...
// Implementation
#include "clang/StaticAnalyzer/Checkers/BuiltinChecker Registration.h"
...
namespace {
	...
	class ValueWalk: publicStmtVisitor<ValueWalk> {
		BugReporter &BR;
		...
		public :
		explicit ValueWalk (BugReporter &B, const CheckerBase *Checker, AnalysisDeclContext *A): BR(B), Checker( Checker), AC(A) {
			insideCallFlag = 0;

			// constsListinit
			constsList.push_back(4);
			constsList.push_back(8);
			constsList.push_back(16);
			constsList.push_back(32);
	...
	// The method for check in potential callexpressions
	void VisitCallExpr(CallExpr *CE);
	void VisitIntegerLiteral(IntegerLiteral *IL);
	};

} // end o f anonimous namespace
...
bool 
ValueWalk::inMemFuncList(std::string name) {
	...
	std::for_each(funcsList.begin(), funcsList.end(), [&name, &funcFlag] (std::strings) {
...
bool 
ValueWalk::inMemConstList(IntegerLiteral *IL) {
...
void 
ValueWalk::ThrowError(Stmt *S) {
	SourceRange R = S−>getSourceRange();
	PathDiagnosticLocation ELoc = PathDiagnosticLocation::createBegin(S, BR.getSourceManager(), AC);
	BR.EmitBasicReport(AC−>getDecl(), Checker, "Usage of a mamory allocator together with a 32−bit constant", 
			"MagicConstantMem", "Usage of a memory allocator together with a 32−bit constant", ELoc, R);
}

void 
ValueWalk:: VisitChildren(Stmt *S) {
...
void 
ValueWalk::VisitCallExpr(CallExpr *CE) {
...
void 
ValueWalk:: VisitIntegerLiteral(IntegerLiteral *IL) {
...
namespace {
	...
	class MagicConstantMem: public Checker<check::ASTCodeBody, check::ASTDecl<FieldDecl>> {

	public:
		void checkASTCodeBody(const Decl *D, AnalysisManager &AM, BugReporter &BR) const;
		void checkASTDecl(const FieldDecl *D, AnalysisManager &AM, BugReporter &BR) const;

	};
} // end anonymous namespace

void 
MagicConstantMem::checkASTCodeBody(const Decl *D, AnalysisManager &AM, BugReporter &BR) const {
	ValueWalk WalkerValue (BR, this, AM.getAnalysisDeclContext(D));
	WalkerValue.Visit(D->getBody());
}

void 
MagicConstantMem::checkASTDecl(constFieldDecl *FD, AnalysisManager &AM, BugReporter &BR) const {
...
void 
ento::registerMagicConstantMem(CheckerManager &Mgr) {
	Mgr.registerChecker<MagicConstantMem>();
}
bool 
ento::shouldRegisterMagicConstantMem(const LangOptions &LO) {}
return true;
